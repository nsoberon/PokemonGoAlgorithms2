...
void Vector<T>::Swap(Nat pos1, Nat pos2) {
    assert(pos1 < longitud && pos2 < longitud);
    Swap(pos1, *this, pos2);
}
...
template<class T>
bool operator==(const Vector<T>& v, const Vector<T>& w) {
    for(Nat p = 0; p < v.Tamanho() && p < w.Tamanho(); ++p)
